-- Fix cart merge function to use maximum quantity instead of adding
-- This prevents quantity doubling during cart merge operations
...
-- FIXED: Using GREATEST instead of addition to prevent quantity doubling
...
BEGIN
    ...
    FOR i IN 0..jsonb_array_length(anonymous_items) - 1 LOOP
        ...
        FOR j IN 0..jsonb_array_length(merged_items) - 1 LOOP
            ...
            -- If same item ID, use maximum quantity instead of adding
...
SELECT 'Cart merge function updated successfully' as status; 