const { Food, Subcategory, CanonicalIngredient, IngredientToCanonical, Recipe, Ingredient } = require('./db/models');
...
async function analyzeWorkflowComparison() {
  try {
    ...
    console.log('   2. messyName → IngredientToCanonical lookup → CanonicalIngredient');
    console.log('   3. CanonicalIngredient → Food products with matching canonicalTag');
    ...
    sampleMappings.forEach(mapping => {
      console.log(`   "${mapping.messyName}" → "${mapping.CanonicalIngredient?.name}"`);
    });
    ...
    console.log('   1. Recipe ingredient → cleanIngredientName() → messyName');
    console.log('   2. messyName → IngredientToCanonical lookup → CanonicalIngredient');
    console.log('   3. CanonicalIngredient → Food products with matching canonicalTag (60-80% coverage)');
    ...
    console.log('   1. Create enrichment script to populate canonicalTag for Food products');
