...
const { Food, CanonicalIngredient, IngredientToCanonical, Recipe, Ingredient } = require('./db/models');
  ...
  try {
    ...
    const recipeStats = await db.query(`
      SELECT 
        ...
        COUNT(DISTINCT CASE WHEN itc."CanonicalIngredientId" IS NOT NULL THEN i.id END) as mapped_ingredients,
      ...
      LEFT JOIN "Food" f ON itc."canonicalName" = f."canonicalTag"
    `, { type: db.QueryTypes.SELECT });
    
    const recipeData = recipeStats[0];
    console.log(`   📖 Total Recipes: ${recipeData.total_recipes.toLocaleString()}`);
    console.log(`   🥘 Total Ingredients: ${recipeData.total_ingredients.toLocaleString()}`);
    console.log(`   🎯 Mapped Ingredients: ${recipeData.mapped_ingredients.toLocaleString()} (${(recipeData.mapped_ingredients/recipeData.total_ingredients*100).toFixed(1)}%)`);
    console.log(`   🛍️  Ingredients with Products: ${recipeData.ingredients_with_products.toLocaleString()} (${(recipeData.ingredients_with_products/recipeData.total_ingredients*100).toFixed(1)}%)`);
