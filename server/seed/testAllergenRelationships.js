...
async function testAllergenRelationships() {
  try {
    ...
    if (shrimpMapping) {
      const shrimpCanonical = await CanonicalIngredient.findByPk(shrimpMapping.CanonicalIngredientId);
    ...
    if (flourMapping) {
      const flourCanonical = await CanonicalIngredient.findByPk(flourMapping.CanonicalIngredientId);
    ...
    if (almondMapping) {
      const almondCanonical = await CanonicalIngredient.findByPk(almondMapping.CanonicalIngredientId);
