...
const { CanonicalIngredient, IngredientToCanonical, Substitution, Food } = require('../db/models');
...
async function audit50RandomRecipes() {
  ...
  const recipes = await Recipe.findAll({ order: [ [Sequelize.literal('RANDOM()')] ], limit: 50, include: [ { model: Ingredient, as: 'Ingredients' } ] });
      ...
      for (const allergen of MAJOR_ALLERGENS) {
        if (canonical.allergens && canonical.allergens.map(a => a.toLowerCase()).includes(allergen)) {
          ...
          for (const sub of subs) {
            const pure = await Food.findOne({ where: { canonicalTag: sub.substituteName.toLowerCase(), canonicalTagConfidence: 'confident' } });
  ...
  console.log(`\nSummary:`);
  console.log(`   Total ingredients checked: ${totalIngredients}`);
