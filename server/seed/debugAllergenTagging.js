const { Food, sequelize } = require('../db/models');
...
async function checkAllergenTagging() {
  ...
  try {
    const productWithWheatFlour = await Food.findOne({
      where: {
        ingredients: {
    ...
    if (productWithWheatFlour) {
      ...
      console.log('Ingredients snippet:', productWithWheatFlour.ingredients.substring(0, 150) + '...');
