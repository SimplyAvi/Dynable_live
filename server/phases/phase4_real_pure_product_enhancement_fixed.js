...
async function phase4RealPureProductEnhancementFixed() {
  console.log('🔄 PHASE 4: Real Pure Product Enhancement (Fixed)\n');
  ...
  try {
    ...
    const canonicalsWithRealProducts = await Food.count({
      where: {
    ...
    for (const substitution of allSubstitutes) {
      const substituteProducts = await Food.count({
        where: {
      ...
      if (substituteProducts === 0) {
        substitutesWithoutProducts.push({
          original: substitution.CanonicalIngredient.name,
    ...
    const substituteProducts = {
      ...
      'flax egg': [
        ...
        'NOW Foods Organic Ground Flax Seeds'
      ],
      'chia egg': [
        'Navitas Organics Chia Seeds',
        'Bob\'s Red Mill Chia Seeds',
        'NOW Foods Organic Chia Seeds'
      ],
    ...
    for (const testCase of testCases) {
      ...
      const originalProducts = await Food.findAll({
        where: {
      ...
      const substituteProducts = await Food.findAll({
        where: {
    ...
    const finalCanonicalsWithRealProducts = await Food.count({
      where: {
        canonicalTagConfidence: 'confident',
        [Op.or]: [
          { brandName: { [Op.ne]: null } },
    ...
    const testIngredients = ['sugar', 'milk', 'almond milk', 'almond flour'];
...
// Run Phase 4
phase4RealPureProductEnhancementFixed().then(() => {
  console.log('\n🏁 Phase 4 complete!');
  process.exit(0);
}).catch(error => {
