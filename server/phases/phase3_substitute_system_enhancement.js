...
async function phase3SubstituteSystemEnhancement() {
  ...
  try {
      ...
      if (patterns) {
        for (const substituteName of patterns) {
          ...
          const existingSubstitution = await Substitution.findOne({
            where: {
              CanonicalIngredientId: missing.canonical,
    ...
    const allSubstitutes = await Substitution.findAll({
      include: [
        { model: CanonicalIngredient, as: 'CanonicalIngredient' },
        { model: CanonicalIngredient, as: 'SubstituteCanonicalIngredient' }
      ]
    });
    ...
    for (const substitution of allSubstitutes) {
      const substituteProducts = await Food.count({
        where: {
          canonicalTag: substitution.SubstituteCanonicalIngredient.name,
      ...
      if (substituteProducts === 0) {
        substitutesWithoutProducts.push({
          ...
          substitute: substitution.SubstituteCanonicalIngredient.name,
