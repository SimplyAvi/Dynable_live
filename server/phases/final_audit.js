...
async function finalAudit() {
  try {
    ...
    const confidentMappings = await Food.count({
      where: {
        canonicalTagConfidence: 'confident'
      }
    });
    
    const suggestedMappings = await Food.count({
      where: {
    ...
    const unmappedRealProducts = await Food.count({
      where: {
        brandOwner: { [db.Sequelize.Op.ne]: 'Generic' },
    ...
    const sampleUnmapped = await Food.findAll({
      where: {
        brandOwner: { [db.Sequelize.Op.ne]: 'Generic' },
        canonicalTag: { [db.Sequelize.Op.or]: [null, ''] }
      },
