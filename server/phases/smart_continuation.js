  ...
  try {
    ...
    for (let batchStart = 0; batchStart < recipes.length; batchStart += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(recipe => limit(() => processRecipeSmart(recipe, mappingCache, unmappedIngredients)))
      );
    ...
    fs.writeFileSync('./unmapped_ingredients_smart.json', JSON.stringify({
      ...
      all: unmappedIngredients.slice(0, 500) // Save first 500 for analysis
    }, null, 2));
    
    console.log(`   💾 Saved ${unmappedIngredients.length} unmapped ingredients to unmapped_ingredients_smart.json`);
...
async function processRecipeSmart(recipe, mappingCache, unmappedIngredients) {
