const { AllergenDerivative, Substitution, Ingredient, IngredientCategorized } = require('./db/models');
...
async function analyzeAllergenSubstituteSystem() {
  try {
    ...
    sampleSubstitutions.forEach(s => {
      const canonicalName = s.Ingredient?.name || 'Unknown';
    ...
    const canonicalsWithAllergens = await Ingredient.findAll({
      where: {
    ...
    // 4. IngredientCategorized product allergen analysis
    console.log('\n📊 FOOD PRODUCT ALLERGENS:');
    const productsWithAllergens = await IngredientCategorized.count({
      where: {
        allergens: { [sequelize.Sequelize.Op.ne]: null }
      }
    });
