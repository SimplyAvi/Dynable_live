...
const { IngredientCategorized, Ingredient, Recipe, Ingredient, IngredientToCanonical } = require('./db/models');
...
class RecipePerfectionWorkflow {
    ...
    async getCurrentStats() {
        ...
        const totalResult = await db.query(`
            SELECT COUNT(*) as count FROM "RecipeIngredients"
        `, { type: Sequelize.QueryTypes.SELECT });
        this.stats.totalRecipeIngredients = totalResult[0].count;
        
        // Get all ingredients and check mappings
        const ingredients = await db.query(`
            SELECT id, name FROM "RecipeIngredients" LIMIT 1000
        `, { type: Sequelize.QueryTypes.SELECT });
            ...
            if (mapping.length > 0) {
                ...
                if (mapping[0].name) {
                    const products = await db.query(`
                        SELECT COUNT(*) as count FROM "IngredientCategorized" 
        ...
        this.stats.mappedRecipeIngredients = mappedCount;
        this.stats.realProducts = realProductCount;
        this.stats.unmappedRecipeIngredients = this.stats.totalRecipeIngredients - this.stats.mappedRecipeIngredients;
        
        console.log(`📈 Current Coverage (Sample of 1000):`);
        console.log(`   Total RecipeIngredients: ${this.stats.totalRecipeIngredients.toLocaleString()}`);
        console.log(`   Mapped RecipeIngredients: ${this.stats.mappedRecipeIngredients.toLocaleString()} (${((this.stats.mappedRecipeIngredients/ingredients.length)*100).toFixed(1)}%)`);
        ...
        console.log(`   Unmapped: ${this.stats.unmappedRecipeIngredients.toLocaleString()}`);
    ...
    async findFrequentUnmappedRecipeIngredients(limit = 50) {
        ...
        const ingredients = await db.query(`
            SELECT id, name FROM "RecipeIngredients" LIMIT 5000
        `, { type: Sequelize.QueryTypes.SELECT });
    ...
    async verifyAllergenFiltering() {
        ...
        // Test gluten products by searching in IngredientCategorized table
    ...
    async runComprehensiveAudit() {
        ...
        for (const recipe of testRecipes) {
            console.log(`\n📖 Recipe: ${recipe.title}`);
            console.log(`   RecipeIngredients: ${recipe.ingredient_count}`);
            
            // Get ingredients for this recipe
            const ingredients = await db.query(`
                SELECT i.name FROM "RecipeIngredients" i WHERE i."RecipeId" = :recipeId
            `, {
