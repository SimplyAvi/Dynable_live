...
const { Ingredient, IngredientToCanonical, IngredientCategorized } = require('./db/models');
const fs = require('fs');
...
async function fastBatchCanonicals() {
  try {
    ...
    const unmappedRecipeIngredients = unmappedText.split('\n').filter(line => line.trim());
    
    console.log(`📊 Found ${unmappedRecipeIngredients.length.toLocaleString()} unmapped ingredients to process.\n`);
    
    // BATCH READ: Get all existing canonical ingredients and mappings in memory
    console.log('📖 Loading existing data into memory...');
    const existingCanonicals = await Ingredient.findAll();
    const existingMappings = await IngredientToCanonical.findAll();
    const existingProducts = await IngredientCategorized.findAll({ 
    ...
    const trulyUnmapped = unmappedRecipeIngredients.filter(ingredient => !mappingMap.has(ingredient));
    
    console.log(`🔄 Resuming from previous run:`);
    console.log(`   - Total ingredients: ${unmappedRecipeIngredients.length.toLocaleString()}`);
    console.log(`   - Already processed: ${(unmappedRecipeIngredients.length - trulyUnmapped.length).toLocaleString()}`);
    ...
    for (let i = 0; i < trulyUnmapped.length; i += BATCH_SIZE) {
      ...
      for (const ingredient of batch) {
        ...
        if (!mappingMap.has(ingredient)) {
          newMappings.push({
            ...
            IngredientId: null // Will be set after canonical creation
          });
      ...
      if (newCanonicals.length >= 1000) {
        ...
        const insertedCanonicals = await Ingredient.bulkCreate(newCanonicals);
        
        // Update mapping IngredientIds
        for (let j = 0; j < insertedCanonicals.length; j++) {
          ...
          if (mapping) {
            mapping.IngredientId = canonical.id;
          }
        }
        
        // Batch insert mappings
        const validMappings = newMappings.filter(m => m.IngredientId);
        ...
        if (newProducts.length > 0) {
          ...
          await IngredientCategorized.bulkCreate(newProducts);
        }
