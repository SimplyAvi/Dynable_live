...
async function optimizedComprehensiveApproach() {
  ...
  try {
    ...
    let existingStats = { totalRecipeIngredients: 0, mappedRecipeIngredients: 0, ingredientsWithRealProducts: 0 };
    ...
    if (fs.existsSync('./progress.json')) {
      ...
      console.log(`   📊 Previous stats: ${existingStats.mappedRecipeIngredients}/${existingStats.totalRecipeIngredients} mapped (${(existingStats.mappedRecipeIngredients/existingStats.totalRecipeIngredients*100).toFixed(1)}%)`);
    }
    ...
    const unmappedRecipeIngredients = [];
    ...
    let totalRecipeIngredients = existingStats.totalRecipeIngredients;
    let mappedRecipeIngredients = existingStats.mappedRecipeIngredients;
    ...
    for (let batchStart = startFromRecipe; batchStart < recipes.length; batchStart += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(recipe => limit(() => processRecipe(recipe, realProductCache, canonicalMappingCache, unmappedRecipeIngredients)))
      );
      ...
      const progress = {
        ...
        stats: { totalRecipeIngredients, mappedRecipeIngredients, ingredientsWithRealProducts },
    ...
    fs.writeFileSync('./unmapped_ingredients.json', JSON.stringify({
      ...
      all: unmappedRecipeIngredients.slice(0, 1000) // Save first 1000 for analysis
    }, null, 2));
    
    console.log(`   💾 Saved ${unmappedRecipeIngredients.length} unmapped ingredients to unmapped_ingredients.json`);
...
async function processRecipe(recipe, realProductCache, canonicalMappingCache, unmappedRecipeIngredients) {
