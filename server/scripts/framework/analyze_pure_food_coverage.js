const { IngredientCategorized, Subcategory, Ingredient } = require('./db/models');
const sequelize = require('./db/database');

async function analyzePureIngredientCategorizedCoverage() {
  try {
    console.log('🔍 ANALYZING PURE FOOD COVERAGE\n');
    
    // 1. Basic IngredientCategorized product statistics
    ...
    const productsWithTags = await IngredientCategorized.findAll({
      where: {
        canonicalTag: { [sequelize.Sequelize.Op.ne]: null }
      },
      limit: 10,
    ...
    const productsWithoutTags = await IngredientCategorized.findAll({
      where: {
        canonicalTag: null
      },
      limit: 10,
    ...
    const productsWithCanonicalAndSubcategory = await IngredientCategorized.findAll({
      where: {
        canonicalTag: { [sequelize.Sequelize.Op.ne]: null }
      },
      include: [{
...
analyzePureIngredientCategorizedCoverage(); 