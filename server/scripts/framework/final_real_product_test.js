...
async function finalRealProductTest() {
  try {
    ...
    let totalRecipeRecipeIngredients = 0;
    ...
    console.log('\n📊 Recipe Test Results:');
    console.log(`  Total ingredients tested: ${totalRecipeRecipeIngredients}`);
    console.log(`  RecipeIngredients with real products: ${ingredientsWithRealProducts} (${((ingredientsWithRealProducts/totalRecipeRecipeIngredients)*100).toFixed(1)}%)`);
    console.log(`  RecipeIngredients with generic products: ${ingredientsWithGenericProducts} (${((ingredientsWithGenericProducts/totalRecipeRecipeIngredients)*100).toFixed(1)}%)`);
    console.log(`  RecipeIngredients with no products: ${ingredientsWithNoProducts} (${((ingredientsWithNoProducts/totalRecipeRecipeIngredients)*100).toFixed(1)}%)`);
    
    // Test common ingredients
    console.log('\n🛍️  Testing Common RecipeIngredients:');
    
    const commonRecipeIngredients = ['salt', 'sugar', 'eggs', 'milk', 'flour', 'butter', 'olive oil', 'cheese'];
    ...
    const recipeRealProductPercentage = (ingredientsWithRealProducts / totalRecipeRecipeIngredients) * 100;
