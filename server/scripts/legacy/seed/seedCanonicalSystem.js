...
async function seedCanonicalSystem() {
  try {
    ...
    // 1. Canonical RecipeIngredients (use findOrCreate for idempotency)
    const [cheddar] = await Ingredient.findOrCreate({
    ...
    const [almondMilk] = await Ingredient.findOrCreate({
    ...
    const [flourWheat] = await Ingredient.findOrCreate({
    ...
    for (const mapping of messyMappings) {
      for (const name of mapping.names) {
        for (const variant of generateVariants(name)) {
          await IngredientToCanonical.findOrCreate({ where: { messyName: variant }, defaults: { IngredientId: mapping.canonicalId } });
        }
      }
    }
    ...
    await Substitution.findOrCreate({
      where: { IngredientId: cheddar.id, substituteName: 'vegan cheddar' },
      defaults: { notes: 'Dairy-free alternative' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: cheddar.id, substituteName: 'nutritional yeast' },
      defaults: { notes: 'Cheesy flavor, vegan' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: almondMilk.id, substituteName: 'soy milk' },
      defaults: { notes: 'Nut-free alternative' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'gluten-free flour blend' },
      defaults: { notes: 'Best 1:1 replacement for baking' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'almond flour' },
      defaults: { notes: 'Wheat-free, nut-based, best for cookies and cakes' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'oat flour' },
      defaults: { notes: 'Wheat-free, gluten-free, best for pancakes and quick breads' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'rice flour' },
      defaults: { notes: 'Wheat-free, best for cakes, may need binder' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'coconut flour' },
      defaults: { notes: 'Wheat-free, absorbs moisture, use less' }
    });
    await Substitution.findOrCreate({
      where: { IngredientId: flourWheat.id, substituteName: 'psyllium husk' },
