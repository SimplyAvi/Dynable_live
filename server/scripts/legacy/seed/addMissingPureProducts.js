const { Recipe, Ingredient, IngredientToCanonical, Ingredient, Substitution, IngredientCategorized } = require('../db/models');
...
async function addMissingPureProducts() {
  try {
    ...
    for (const ing of ingredients) {
      ...
      let canonical = null;
      if (mapping) canonical = await Ingredient.findByPk(mapping.IngredientId);
      if (!canonical) continue;
      // Add pure product for canonical ingredient if missing
      if (!added.has(canonical.name.toLowerCase())) {
        const existing = await IngredientCategorized.findOne({ where: { canonicalTag: canonical.name.toLowerCase(), canonicalTagConfidence: 'confident' } });
        if (!existing) {
      ...
      for (const sub of subs) {
        if (!added.has(sub.substituteName.toLowerCase())) {
          const existingSub = await IngredientCategorized.findOne({ where: { canonicalTag: sub.substituteName.toLowerCase(), canonicalTagConfidence: 'confident' } });
          if (!existingSub) {
            await IngredientCategorized.create({
