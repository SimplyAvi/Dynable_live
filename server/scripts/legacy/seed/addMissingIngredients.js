...
async function addMissingRecipeIngredients() {
    console.log('➕ Adding Missing RecipeIngredients to Canonical Database\n');
    ...
    try {
        ...
        for (const name of newRecipeIngredients) {
            ...
            if (existing) {
                ...
                await IngredientToCanonical.findOrCreate({
                    ...
                    defaults: { IngredientId: newIngredient.id }
                });
            }
        }
        ...
        console.log(`Total canonical ingredients: ${allRecipeIngredients.length}`);

        // Show the newly added ingredients
        const newIngredientNames = newRecipeIngredients;
        console.log(`\n📋 Newly added ingredients:`);
        for (const name of newIngredientNames) {
            const match = allRecipeIngredients.find(ci => ci.name === name);
            if (match) {
