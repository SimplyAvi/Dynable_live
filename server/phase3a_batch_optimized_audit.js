...
async function phase3aBatchOptimizedAudit() {
  ...
  const allMappings = await db.query(`
    SELECT "messyName", "CanonicalIngredientId" FROM "IngredientToCanonicals"
  `, { type: Sequelize.QueryTypes.SELECT });
  const mappingMap = new Map();
  allMappings.forEach(m => mappingMap.set(m.messyName.toLowerCase(), m.CanonicalIngredientId));

  // 2. Preload all canonicals (id → name)
  const allCanonicals = await db.query(`
    SELECT id, name FROM "CanonicalIngredients"
  `, { type: Sequelize.QueryTypes.SELECT });
