...
async function testRealProductImpact() {
  ...
  try {
    ...
    console.log(`   🏭 Ingredients with Generic Products: ${ingredientsWithGenericProducts} (${(ingredientsWithGenericProducts/totalIngredients*100).toFixed(1)}%)`);
    console.log(`   ❌ Unmapped Ingredients: ${totalIngredients - mappedIngredients} (${((totalIngredients - mappedIngredients)/totalIngredients*100).toFixed(1)}%)`);
    
    // Show top ingredients by real product coverage
    console.log('\n🏆 TOP INGREDIENTS BY REAL PRODUCT COVERAGE:');
    const sortedIngredients = Object.entries(ingredientCoverage)
    ...
    const overallRealCoverage = (ingredientsWithRealProducts / totalIngredients * 100).toFixed(1);
