const { Food, Subcategory, CanonicalIngredient, IngredientToCanonical } = require('./db/models');
...
class OptimizedIngredientEnrichment {
  ...
  async initialize() {
    ...
    const canonicals = await CanonicalIngredient.findAll({
    ...
    for (const mapping of mappings) {
      if (mapping.messyName && mapping.CanonicalIngredient) {
        this.mappingCache.set(mapping.messyName.toLowerCase(), mapping.CanonicalIngredient);
      }
    }
  ...
  async validateResults() {
    ...
    // Show sample results
    const sampleProducts = await Food.findAll({
      where: { 
