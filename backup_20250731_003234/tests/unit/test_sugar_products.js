...
async function testSugarProducts() {
    ...
    try {
        ...
        const response = await axios.get('http://process.env.API_URL || 'localhost:5001'/api/product/foods?page=1&limit=100');
        ...
        // Test with sugar
        try {
            const sugarResponse = await axios.post('http://process.env.API_URL || 'localhost:5001'/api/product/by-ingredient', {
