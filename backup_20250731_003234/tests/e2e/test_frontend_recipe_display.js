...
async function testFrontendRecipeDisplay() {
    ...
    try {
        ...
        const recipeResponse = await axios.get('http://process.env.API_URL || 'localhost:5001'/api/recipe/?id=1');
        ...
        if (testIngredient) {
            ...
            const productResponse = await axios.post('http://process.env.API_URL || 'localhost:5001'/api/product/by-ingredient', {
        ...
        for (const ingredient of basicIngredients) {
            try {
                const response = await axios.post('http://process.env.API_URL || 'localhost:5001'/api/product/by-ingredient', {
        ...
        const allergenResponse = await axios.post('http://process.env.API_URL || 'localhost:5001'/api/product/by-ingredient', {
