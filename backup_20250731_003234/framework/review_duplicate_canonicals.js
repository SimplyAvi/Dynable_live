...
async function reviewDuplicateCanonicals() {
  ...
  try {
    ...
    canonicals.forEach(canonical => {
      ...
      } else if (realRecipeIngredients.includes(name)) {
        categories.realRecipeIngredients.push(canonical);
      } else {
    ...
    console.log(`🥕 Real ingredients (review for variants): ${categories.realRecipeIngredients.length}`);
    ...
    if (categories.realRecipeIngredients.length > 0) {
      console.log('🥕 TOP REAL INGREDIENTS (REVIEW FOR VARIANTS):');
      categories.realRecipeIngredients.slice(0, 10).forEach(canonical => {
        console.log(`   "${canonical.canonicalName}": ${canonical.mappingCount} mappings`);
        canonical.messyNames.slice(0, 5).forEach(messy => {
          console.log(`     - "${messy}"`);
        });
    ...
    const totalRealMappings = categories.realRecipeIngredients.reduce((sum, c) => sum + parseInt(c.mappingCount), 0);
