...
async function main() {
  ...
  // 2. Batch canonical creation
  let added = 0, skipped = 0, newCanonicals = [];
  for (const c of toAdd) {
    try {
      await db.query(
        'INSERT INTO "CanonicalRecipeIngredients" ("name", "category", "createdAt", "updatedAt") VALUES (:name, :category, NOW(), NOW())',
        { replacements: { name: c.canonical, category: c.category } }
      );
      added++;
