/**
 * Direct Database Analysis Script for Dynable
...
console.log('🔍 DYNABLE DIRECT DATABASE ANALYSIS STARTING...');
console.log('===============================================');
...
async function analyzeTableStructure() {
    ...
    try {
        // Known tables based on code analysis
        const knownTables = [
        ...
        for (const tableName of knownTables) {
            await analyzeTableColumns(tableName);
        }
...
async function analyzeTableColumns(tableName) {
    try {
        ...
        const columns = sampleData.length > 0 ? Object.keys(sampleData[0]).map(key => ({
            column_name: key,
            data_type: typeof sampleData[0][key],
            is_nullable: true // We'll determine this from data
        })) : [];
...
async function analyzeIngredientCategorized() {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('IngredientCategorized')
            .select('id, description, allergens, ingredients, brandName, canonicalTag, seller_id, is_active');
        ...
        const analysis = {
            ...
            hasCanonicalTag: data.filter(row => row.canonicalTag).length,
            hasSellerId: data.filter(row => row.seller_id).length,
            ...
            activeProducts: data.filter(row => row.is_active).length,
            inactiveProducts: data.filter(row => !row.is_active).length
        };
        
        // Sample data with allergens
        const sampleWithAllergens = data.filter(row => row.allergens && row.allergens.length > 0).slice(0, 10);
        
        // Get unique allergen variations
        ...
        analysis.uniqueAllergenVariations = Array.from(uniqueAllergens).sort();
        
        analysisResults.dataQuality.IngredientCategorized = {
            ...analysis,
            samples: sampleWithAllergens
        };
        ...
        console.log(`   📊 Has canonical tag: ${analysis.hasCanonicalTag}`);
        console.log(`   📊 Has seller ID: ${analysis.hasSellerId}`);
        console.log(`   📊 Null allergens: ${analysis.nullAllergens}`);
        console.log(`   📊 Null ingredients: ${analysis.nullIngredients}`);
        console.log(`   📊 Active products: ${analysis.activeProducts}`);
        console.log(`   📊 Inactive products: ${analysis.inactiveProducts}`);
        console.log(`   📊 Unique allergen variations: ${analysis.uniqueAllergenVariations.length}`);
...
async function analyzeUsers() {
    ...
    try {
        ...
        const analysis = {
            ...
            usersWithSupabaseId: data.filter(row => row.supabase_user_id).length
        };
        ...
        console.log(`   📊 Users with Supabase ID: ${analysis.usersWithSupabaseId}`);
...
async function analyzeRLSPolicies() {
    ...
    try {
        // Based on migration files, we know the RLS policies
        analysisResults.rlsPolicies = {
            status: 'ANALYZED_FROM_MIGRATIONS',
            policies: [
                {
                    table: 'Carts',
...
async function analyzeRelationships() {
    ...
    try {
        // Based on code analysis, we know the relationships
        analysisResults.relationships = {
            status: 'ANALYZED_FROM_CODE',
            relationships: [
                {
                    table: 'IngredientCategorized',
                    column: 'seller_id',
...
async function analyzeIndexes() {
    ...
    try {
        // Based on performance issues mentioned, we know indexes are missing
        analysisResults.indexes = {
            status: 'ANALYZED_FROM_PERFORMANCE_ISSUES',
            missingIndexes: [
                {
                    table: 'IngredientCategorized',
                    column: 'allergens',
                    type: 'GIN',
                    reason: 'Allergen filtering performance',
                    priority: 'CRITICAL'
                },
                {
                    table: 'IngredientCategorized',
                    column: 'description',
                    type: 'GIN',
                    reason: 'Text search performance',
                    priority: 'HIGH'
                },
                {
                    table: 'IngredientCategorized',
                    column: 'brandName',
                    type: 'BTREE',
                    reason: 'Brand filtering performance',
                    priority: 'MEDIUM'
                }
            ]
        };
...
async function identifyDataInconsistencies() {
    ...
    try {
        ...
        if (analysisResults.dataQuality.IngredientCategorized) {
            const allergenVariations = analysisResults.dataQuality.IngredientCategorized.uniqueAllergenVariations;
            
            if (allergenVariations && allergenVariations.length > 0) {
                inconsistencies.push({
                    type: 'ALLERGEN_NAMING_INCONSISTENCIES',
                    count: allergenVariations.length,
                    examples: allergenVariations.slice(0, 10),
...
function printSummary() {
    ...
    if (analysisResults.dataQuality.IngredientCategorized) {
        ...
        console.log(`   - Unique allergen variations: ${stats.uniqueAllergenVariations ? stats.uniqueAllergenVariations.length : 0}`);
    }
