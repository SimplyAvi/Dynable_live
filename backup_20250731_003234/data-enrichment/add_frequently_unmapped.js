...
async function addFrequentlyUnmappedMappings() {
  ...
  for (const mapping of ALL_INGREDIENTS_TO_ADD) {
    ...
    try {
      ...
      const existingMapping = await IngredientToCanonical.findOne({
        where: { messyName: ingredient, IngredientId: canonical.id }
      });
      if (!existingMapping) {
  ...
  const testRecipeIngredients = ['agave nectar', 'almond butter', 'smoked paprika', 'bay leaves'];
  for (const testIngredient of testRecipeIngredients) {
    ...
    if (mapping && mapping.IngredientId) {
      ...
      const products = await IngredientCategorized.count({
