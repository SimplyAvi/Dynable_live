...
function analyzeIngredientMatch(description, ingredientName) {
  ...
  // VALIDATION: Skip invalid canonical ingredients
  const BLACKLIST = ['ice', 'pie', 'up', 'chopped', 'ed', 'ing', 'ly', 'er', 'est'];
  if (BLACKLIST.includes(ingredientName.toLowerCase())) {
    return { match: false, reason: 'blacklisted_ingredient', confidence: 'none' };
  }
  
  // VALIDATION: Minimum length check
  if (ingredientName.length < 3) {
    return { match: false, reason: 'too_short', confidence: 'none' };
  }
  ...
  // Check for word boundary match (lowest confidence)
  const wordBoundaryRegex = new RegExp(`\\b${ingredientName.toLowerCase()}\\b`, 'i');
  if (wordBoundaryRegex.test(desc)) {
    return { match: true, reason: 'word_boundary_match', confidence: 'low' };
  }
...
async function suggestProductCanonicalTags() {
  ...
  const canonicalRecipeIngredients = await Ingredient.findAll();
  const canonicalNames = canonicalRecipeIngredients.map(ci => ci.name.toLowerCase());
  const allAliases = canonicalRecipeIngredients.flatMap(ci => (ci.aliases || []).filter(a => a !== null).map(a => a.toLowerCase()));
  const allTags = Array.from(new Set([...canonicalNames, ...allAliases]));
  
  console.log(`📊 Found ${canonicalRecipeIngredients.length} canonical ingredients with ${allTags.length} total tags`);
  ...
  for (const product of products) {
    // VALIDATION: Skip generic "Pure" products
    if (product.brandOwner === 'Generic' && 
        product.description.toLowerCase().startsWith('pure')) {
      continue;
    }
