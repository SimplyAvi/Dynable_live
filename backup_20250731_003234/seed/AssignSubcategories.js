const { Subcategory, Ingredient, IngredientCategorized } = require('../db/models');
...
async function assignSubcategories() {
  console.time('assignSubcategoriesRecipeIngredients'); // Start the timer
  try{
    let counter = 0
    ...
    console.timeEnd('assignSubcategoriesRecipeIngredients');
  ...
  try{
    ...
    console.log(`${counter} Subcategories assigned to products(IngredientCategorizeds).`);
  } catch(error){
    console.error(error)
  }
}
