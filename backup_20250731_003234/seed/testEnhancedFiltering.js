const { IngredientCategorized, Subcategory } = require('../db/models');
  ...
  try {
    ...
    for (const ingredient of basicRecipeIngredients) {
      ...
      for (const subcat of pureSubcategories) {
        const count = await IngredientCategorized.count({
      ...
      const filteredProducts = await IngredientCategorized.findAll(enhancedQuery);
    ...
    for (const brand of problematicBrands) {
      const brandProducts = await IngredientCategorized.findAll({
        where: {
    ...
    const enhancedSugarProducts = await IngredientCategorized.findAll(sugarQuery);
    ...
    const basicSugarProducts = await IngredientCategorized.findAll(basicSugarQuery);
