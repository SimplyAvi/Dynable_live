...
async function addMissingMappings() {
    ...
    try {
        ...
        const yeast = await Ingredient.findOne({ where: { name: 'yeast' } });
        const water = await Ingredient.findOne({ where: { name: 'water' } });
        const mozzarella = await Ingredient.findOne({ where: { name: 'mozzarella cheese' } });
        const cookingSpray = await Ingredient.findOne({ where: { name: 'cooking spray' } });
        ...
        for (const mapping of missingMappings) {
            ...
            if (existing) {
                ...
                await IngredientToCanonical.create({
                    ...
                    IngredientId: mapping.canonicalId
                });
        ...
        for (const testIngredient of testRecipeIngredients) {
            ...
            if (mapping) {
                console.log(`   ✅ "${testIngredient}" → "${mapping.Ingredient.name}"`);
            } else {
