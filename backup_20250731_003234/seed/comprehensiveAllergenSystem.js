...
async function seedComprehensiveAllergenSystem() {
  try {
    ...
    // 1. Create Canonical RecipeIngredients for all major allergen categories
    const canonicalRecipeIngredients = await Promise.all([
      // Dairy/Milk
    ...
    const [
      ...
      soybean, tofu,
      sesame
    ] = canonicalRecipeIngredients.map(([ingredient]) => ingredient);
      ...
      for (const name of mapping.names) {
        for (const variant of generateVariants(name)) {
          await IngredientToCanonical.findOrCreate({ 
            ...
            defaults: { IngredientId: mapping.canonicalId } 
          });
        }
      }
    }
