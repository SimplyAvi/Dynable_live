...
// FIXED classification rules based on your advisor's examples
const FIXED_CLASSIFICATION_RULES = {
    ...
    RAW_INGREDIENTS: {
        ...
        // Must be BASIC versions only
        requirements: [
            'no flavoring', 'no seasoning', 'no preparation',
            'no combining', 'no processing', 'no additives'
        ]
    },
    ...
    PROCESSED_INDICATORS: [
        ...
        'spicy', 'italian', 'garlic', 'herb', 'seasoned', 'flavored',
...
function classifyProductFixed(productName) {
    ...
    const hasProcessedIndicators = FIXED_CLASSIFICATION_RULES.PROCESSED_INDICATORS.some(indicator => 
    ...
    const hasRawIngredients = FIXED_CLASSIFICATION_RULES.RAW_INGREDIENTS.keywords.some(keyword => 
...
// Test the fixed classification with your advisor's examples
function testAdvisorExamples() {
    console.log('🧪 Testing Fixed Classification with Advisor Examples...\n');
    ...
    advisorExamples.raw.forEach(example => {
        const classification = classifyProductFixed(example);
...
// Test classification on 1,000 products with FIXED logic
async function testFixedClassificationOnBatch() {
    console.log('🧪 Testing FIXED Classification on 1,000 Products...\n');
    ...
    try {
        ...
        console.log(`📊 Processing ${products.length} products with FIXED classification...\n`);
        ...
        for (const product of products) {
            const classification = classifyProductFixed(product.canonical_product_name);
        ...
        console.error('❌ Error in fixed classification test:', error);
        return null;
    }
}

// Main test function
async function runFixedClassificationTest() {
    console.log('🚀 Starting FIXED Classification Test...\n');
    
    // Step 1: Test advisor examples
    testAdvisorExamples();
    
    // Step 2: Test on real data
    const classifications = await testFixedClassificationOnBatch();
    
    if (classifications) {
        console.log('🎉 FIXED Classification Test Complete!');
        console.log('\n📋 Key Improvements:');
        console.log('   ✅ Processed indicators take priority');
        console.log('   ✅ Seasoned/flavored items → PROCESSED');
        console.log('   ✅ Basic ingredients → RAW');
        console.log('   ✅ Matches advisor examples');
        console.log('\n💡 Ready to implement with corrected logic!');
    }
}

runFixedClassificationTest().catch(console.error); 