...
// Resume from specific offset (default: 79000 based on logs)
const RESUME_FROM_OFFSET = process.env.RESUME_OFFSET ? parseInt(process.env.RESUME_OFFSET) : 79000;
...
async function getTotalProductCount() {
    try {
        ...
        // Method 2: Get max ID as approximation
        ...
        // Method 3: Use a known value from previous runs
        console.log('🔄 Using fallback count from previous runs...');
        return 243114; // Known total from previous runs
        
    } catch (error) {
        console.error('❌ Error getting product count:', error);
        console.log('🔄 Using fallback count: 243114');
        return 243114; // Fallback to known total
    }
}
...
async function saveCheckpoint(offset, processedCount, totalCount) {
    try {
        ...
        const fs = require('fs');
        fs.writeFileSync('scripts/logs/resume_checkpoint.json', JSON.stringify(checkpoint, null, 2));
...
async function resumeProductMapping() {
    console.log('🚀 Resuming Product Canonical Mapping...');
