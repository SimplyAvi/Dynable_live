...
async function testRecipeRange(startId, endId, sampleSize) {
  ...
  for (const recipe of recipes) {
    const ingredients = await db.query(`
      SELECT i.name, i."cleanedName", f."canonicalTag", f."isPureIngredient"
      FROM "Ingredients" i
      LEFT JOIN "Food" f ON LOWER(REPLACE(i."cleanedName", ' ', '')) = LOWER(REPLACE(f."canonicalTag", ' ', ''))
