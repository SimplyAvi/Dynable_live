-- 🎯 MINIMAL MIGRATION - VERY SMALL BATCHES TO AVOID TIMEOUTS
-- Process data in tiny batches to prevent any timeouts
...
-- Step 3: Handle AllergenDerivatives safely (small table, direct approach)
...
-- Step 4: Update SearchPreferences (small table, direct update)
...
-- Step 5: Update Ingredients in very small batches (50 rows)
DO $$
DECLARE
...
BEGIN
    LOOP
        ...
        RAISE NOTICE 'Updated % rows in Ingredients (total: %, iteration: %)', batch_updated, total_updated, iteration_count;
        
        -- Exit if we've processed enough
        IF total_updated >= 10000 THEN
            RAISE NOTICE 'Reached 10,000 rows, stopping for safety';
            EXIT;
        END IF;
    END LOOP;
END $$;

-- Step 6: Update IngredientCategorized in very small batches (25 rows)
DO $$
DECLARE
    ...
    batch_updated INTEGER;
    offset_val INTEGER := 0;
    max_iterations INTEGER := 2000; -- Safety limit
    iteration_count INTEGER := 0;
BEGIN
    LOOP
        iteration_count := iteration_count + 1;
        ...
        RAISE NOTICE 'Updated % rows in IngredientCategorized (total: %, iteration: %)', batch_updated, total_updated, iteration_count;
        
        -- Exit if we've processed enough
        IF total_updated >= 50000 THEN
            RAISE NOTICE 'Reached 50,000 rows, stopping for safety';
            EXIT;
        END IF;
    END LOOP;
END $$;
